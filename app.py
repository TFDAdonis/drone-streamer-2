...
def get_video_thumbnail_base64(filepath):
    """Extract first frame from video and convert to base64"""
    try:
        if filepath and os.path.exists(filepath):
...
def create_story_marker(item):
    ...
    has_video = filepath and os.path.exists(filepath) and item['type'] == 'video'
    ...
    if has_video:
        thumb_base64 = get_video_thumbnail_base64(filepath)
        if thumb_base64:
            html = f'''
            <div style="
                width: 60px;
...
# Admin Authentication Section - Hidden Icon
